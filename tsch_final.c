...
void eval(char *cmdline)
{
	...
	/* Parent process */
    		...
    		if (bg == 1) {
    			...
    			if(sigprocmask(SIG_UNBLOCK, &mask, 0)<0) unix_error("sigprocmask error");
			 waitfg(pid);
    		}
    	}
    }
...
void do_bgfg(char **argv) 
{
    ...
    if (id != NULL) {     /* if arguments are passed to bg or fg */
	...
	} else if (isdigit(id[0])) {	/* is it a process id? */
	    pid = atoi(&id[0]);
  ...
  if(!strcmp(argv[0], "bg")){
    ...
    printf("[%d] (%d) %s", do_job->jid, do_job->pid, do_job->cmdline);
    do_job->state=BG;
    pid=do_job->pid;
...
void sigtstp_handler(int sig)
{
    ...
    /* If there is foreground job, then send the signal */
    if (fgpid(jobs) != 0) {
	if(kill(-pid, SIGTSTP)<0 ) unix_error("kill SIGTSTP error");
    }
